...
export default function Proposal() {
  ...
  const submit = () => {
    if(title && textArea && choiceData && end && start !== "NaN" && textArea !== "" && itemValue === false){
      voting(userAddress, title, textArea, choiceData, start, end,setNav);
      setNav(false)
    }else{
  ...
  const checkIfDuplicateExists=(choiceData:any)=> {
    return new Set(choiceData.split(",")).size !== choiceData.split(",").length
  }
  useEffect(() => {
    const checkIfDuplicate:any =  checkIfDuplicateExists(choiceData)

    setItemValue(checkIfDuplicate);
  }, [choiceData]);

  useEffect(() => {
    connectWallet().then((d: any) => {
  ...
  useEffect(() => {
    if (quill) {
      quill.on('text-change', () => {
        ...
        const quilData = quill.root.innerHTML;
        console.log(quilData.length,"quildatdaggfasdcsf..................a")
        setTextArea(quilData)

        if (quilData.length >= 8) {
          setTextAreaStatus(true)
        } else if (quilData.length == 1) {
            ...
            {textAreaStatus ? (
              <div className="textArea">
                ...
                {/* <textarea
                  ...
                  disabled
                ></textarea> */}
                <p className="textA" dangerouslySetInnerHTML={{__html: textArea}}></p>
              </div>
            ) : (
            ...
            <div className="card" style={{ width: '22rem' }}>
              ...
              <div className="cardContent">
                <div className="actionInputDiv">
                  ...
                  <input
                    ...
                    onChange={(e: any) => setStartDate(e.target.value?.split('-'))}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    min={new Date().toISOString().split('T')[0]}
                    onChange={(e: any) => setEndDate(e.target.value?.split('-'))}
                  />
                </div>
              ...
              <input type="time" name="" id="" className="actionsInput" />
                ...
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                  ...
                  {chainId == 1807 && userAddress ? (
                    <button className="actionBtn1" onClick={submit}>
                      Submit
                    </button>
                  ) : (
                    <button
